/**
 * Context Tracker for LLM Conversations
 * 
 * This module manages the context window for conversations with Large Language Models (LLMs).
 * It helps prevent token limit overflows by:
 * 1. Tracking token usage in conversations
 * 2. Estimating when the context window is getting full
 * 3. Intelligently truncating message history when needed
 * 
 * The tracker works in two phases:
 * - Pre-request: Uses word count to estimate if truncation is needed
 * - Post-request: Gets actual token counts from the LLM response
 * 
 * Key features:
 * - Model-aware token limits
 * - Preserves system messages and recent context
 * - Supports various LLM providers (OpenAI, Anthropic, etc.)
 * - Tracks cached tokens for optimization
 */
