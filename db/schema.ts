...
export const users = pgTable("users", {
	...
	pgPolicy("Users can manage their own data", { as: "restrictive", for: "all", to: ["public"], using: sql`auth.uid()::text = id`, withCheck: sql`auth.uid()::text = id` }),
]);
...
export const messages = pgTable("messages", {
	...
	pgPolicy("Users can manage their own messages", { as: "restrictive", for: "all", to: ["public"], using: sql`auth.uid()::text = user_id`, withCheck: sql`auth.uid()::text = user_id` }),
...
export const activeResources = pgTable("active_resources", {
	...
	pgPolicy("Users can read their own active resources", { as: "restrictive", for: "select", to: ["public"], using: sql`auth.uid()::text = user_id` }),
	pgPolicy("Users can insert their own active resources", { as: "restrictive", for: "insert", to: ["public"], withCheck: sql`auth.uid()::text = user_id` }),
	pgPolicy("Users can update their own active resources", { as: "restrictive", for: "update", to: ["public"], using: sql`auth.uid()::text = user_id`, withCheck: sql`auth.uid()::text = user_id` }),
	pgPolicy("Users can delete their own active resources", { as: "restrictive", for: "delete", to: ["public"], using: sql`auth.uid()::text = user_id` }),
]);
...
export const activeTools = pgTable("active_tools", {
	...
	pgPolicy("Users can read their active tools", { as: "restrictive", for: "select", to: ["public"], using: sql`EXISTS (
		...
		AND active_resources.user_id = auth.uid()::text
	)` }),
...
export const resourceCache = pgTable("resource_cache", {
	...
	pgPolicy("Anyone can read cache", { as: "restrictive", for: "select", to: ["public"], using: sql`auth.uid()::text = user_id` }),
	pgPolicy("Anyone can manage cache", { as: "restrictive", for: "all", to: ["public"], using: sql`auth.uid()::text = user_id`, withCheck: sql`auth.uid()::text = user_id` }),
]);
