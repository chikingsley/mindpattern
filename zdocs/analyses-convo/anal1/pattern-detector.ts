...
class PatternDetector {
  ...
  private async analyzeMessage(
    ...
    // Detect n-grams (phrases)
    const ngrams = this.extractNgrams(words, 3); // Up to trigrams
    
    // Find similar phrases
    for (const phrase of ngrams) {
      const key = phrase.join(' ');
      if (!this.phraseClusters.has(key)) {
        this.phraseClusters.set(key, new Set());
      }
      this.phraseClusters.get(key)?.add(content);
    }
